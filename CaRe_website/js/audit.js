...
async function generateWorkloadChart() {
    ...
    const response = await fetch('../API/comptoller_api.php?method=getTherapistWorkload');
...
async function generatePatientStatisticsChart() {
    ...
    const response = await fetch('../API/comptoller_api.php?method=getPatientStats');
    const data = await response.json();
...
async function generateSessionDurationChart() {
    ...
    const response = await fetch('../API/comptoller_api.php?method=getAppointmentDurationStats');
    const data = await response.json();
    const labels = data.map(item => item.therapist_name);
    const totalDurations = data.map(item => item.total_duration);
